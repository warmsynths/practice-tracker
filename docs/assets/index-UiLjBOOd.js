...
`,we=[1,3,7,14,30],Ue=5;function Dl(s,e){const t=X(s).getTime(),r=X(e).getTime();return Math.round((t-r)/(1e3*60*60*24))}function jl(s,e,t=new Date){const r=X(t),n=e.filter(_=>_.instrumentId===s&&!_.deletedAt).sort((_,b)=>new Date(_.start).getTime()-new Date(b.start).getTime());if(n.length===0)return{instrumentId:s,step:0,maxSteps:Ue,intervalDays:we[0],lastPracticedAt:null,nextDueAt:null,daysRemaining:0,status:"new",isDueToday:!1,isOverdue:!1,cycleCount:0,label:"New · Ready for Step 1"};const i=[],a=new Set;for(const _ of n){const b=X(new Date(_.start)),E=Se(b);a.has(E)||(a.add(E),i.push(b))}let o=0,l=null,c=0;for(let _=0;_<i.length;_++){const b=i[_];if(_===0){o=1;const E=we[0];l=D(b,E)}else if(l?b.getTime()<=l.getTime():!0)if(o>=Ue){c++,o=1;const O=we[0];l=D(b,O)}else{o++;const O=we[o-1];l=D(b,O)}else{o=1;const O=we[0];l=D(b,O)}}const d=n[n.length-1].start,u=l?l.toISOString():null,h=we[Math.max(0,Math.min(o-1,Ue-1))],p=l?Dl(l,r):0,f=p<0,g=p===0;let y="cold";f?y="overdue":g?y="due":p===1?y="hot":p<=3?y="warm":p<=7?y="cool":y="cold";let w;return f?w=`Overdue by ${Math.abs(p)}d (was Step ${o}: ${h}d)`:g?w=`Due today · Step ${o}/${Ue} (${h}d)`:p===1?w=`Due tomorrow · Step ${o}/${Ue} (${h}d)`:w=`Step ${o}/${Ue} · Due in ${p}d (${h}d)`,{instrumentId:s,step:o,maxSteps:Ue,intervalDays:h,lastPracticedAt:d,nextDueAt:u,daysRemaining:p,status:y,isDueToday:g,isOverdue:f,cycleCount:c,label:w}}function Dn(s,e,t=new Date){const r=new Map;for(const n of s)r.set(n.id,jl(n.id,e,t));return r}function Fl(s,e,t){const r=s.isDueToday||s.isOverdue||s.status==="due"||s.status==="overdue",n=s.lastPracticedAt&&Nl(new Date(s.lastPracticedAt),new Date);return r?{badgeText:"Due",badgeBg:"#B4543C",badgeColor:"#FBF6F3",badgeDot:"#FBF6F3",isDue:!0}:n?{badgeText:`${Math.max(1,s.step)}/5`,badgeBg:e?"rgba(255, 255, 255, 0.22)":"#EFEEE9",badgeColor:e?"#F5F2F6":"#767668",badgeDot:e?"#F5F2F6":t,isDue:!1}:{badgeText:`${Math.max(0,s.daysRemaining)}d`,badgeBg:e?"rgba(255, 255, 255, 0.22)":"#EFEEE9",badgeColor:e?"#F5F2F6":"#767668",badgeDot:"#A3A297",isDue:!1}}function Nl(s,e){return s.getFullYear()===e.getFullYear()&&s.getMonth()===e.getMonth()&&s.getDate()===e.getDate()}var Ll=Object.defineProperty,Ul=Object.getOwnPropertyDescriptor,Ae=(s,e,t,r)=>{for(var n=r>1?void 0:r?Ul(e,t):e,i=s.length-1,a;i>=0;i--)(a=s[i])&&(n=(r?a(e,t,n):a(n))||n);return r&&n&&Ll(e,t,n),n};let ae=class extends G{constructor(){super(...arguments),this.instruments=[],this.sessions=[],this.activeSession=null,this.now=Date.now(),this.logOpen=!1,this.logInstId=null,this.logDuration=15,this.logDayOffset=1}handleStart(s){this.dispatchEvent(new CustomEvent("start-session",{detail:{instrumentId:s},bubbles:!0,composed:!0}))}handleEnd(){this.dispatchEvent(new CustomEvent("end-session",{bubbles:!0,composed:!0}))}handleDiscard(){this.dispatchEvent(new CustomEvent("discard-session",{bubbles:!0,composed:!0}))}handleOpenManual(){this.dispatchEvent(new CustomEvent("open-manual-log",{bubbles:!0,composed:!0}))}handleSaveQuickLog(){const s=this.logInstId||this.instruments[0]&&this.instruments[0].id;s&&(this.dispatchEvent(new CustomEvent("quick-log-session",{detail:{instrumentId:s,duration:this.logDuration,daysAgo:this.logDayOffset},bubbles:!0,composed:!0})),this.logOpen=!1)}render(){const s=X(new Date),{currentStreak:e,consistency30d:t}=di(this.sessions),r=Ds("#6B7F6E",t),n=this.sessions.filter(u=>ui(new Date(u.start),s)),i=new Map(this.instruments.map(u=>[u.id,u])),a=Dn(this.instruments,this.sessions,s),o=this.activeSession?i.get(this.activeSession.instrumentId)||{id:this.activeSession.instrumentId,name:"Instrument",color:"#6B7F6E"}:null;let l="0:00",c="#6B7F6E";if(this.activeSession&&o){const u=Math.max(0,this.now-this.activeSession.startedAt),h=Math.floor(u/6e4),p=Math.floor(u%6e4/1e3);l=`${h}:${String(p).padStart(2,"0")}`;const f=Math.min(15,h*1.1).toFixed(1),g=Math.max(46,82-h*2.2).toFixed(0);c=`radial-gradient(135% 115% at 22% 0%, color-mix(in oklab, ${o.color}, white ${f}%) 0%, ${o.color} ${g}%)`}const d=this.logInstId||this.instruments[0]&&this.instruments[0].id;return v`
          ...
          ${this.activeSession&&o?v`
              ...
              `:v`
                ...
                <div class="chips-wrap">
                  ${this.instruments.map((u,h)=>{const p=a.get(u.id),f=u.tier==="primary",g=p?Fl(p,f,u.color):{badgeText:"1d",badgeBg:f?"rgba(255, 255, 255, 0.22)":"#EFEEE9",badgeColor:f?"#F5F2F6":"#767668",badgeDot:"#A3A297",isDue:!1};return v`
                      <div
                        data-lift
    ...
    `];Pe([P({type:Array})],de.prototype,"instruments",2);Pe([P({type:Array})],de.prototype,"sessions",2);Pe([A()],de.prototype,"addOpen",2);Pe([A()],de.prototype,"addName",2);Pe([A()],de.prototype,"addColor",2);Pe([A()],de.prototype,"addTier",2);Pe([A()],de.prototype,"tiltMap",2);de=Pe([pe("pt-kit-view")],de);var zl=Object.defineProperty,Hl=Object.getOwnPropertyDescriptor,zt=(s,e,t,r)=>{for(var n=r>1?void 0:r?Hl(e,t):e,i=s.length-1,a;i>=0;i--)(a=s[i])&&(n=(r?a(e,t,n):a(n))||n);return r&&n&&zl(e,t,n),n};let Je=class extends G{constructor(){super(...arguments),this.instruments=[],this.sessions=[],this.scale="week",this.instFilter=[]}toggleInstFilter(s){const e=this.instFilter||[],t=e.includes(s)?e.filter(r=>r!==s):e.concat([s]);this.instFilter=t.length===this.instruments.length?[]:t}clearInstFilter(){this.instFilter=[]}handleEditSession(s){this.dispatchEvent(new CustomEvent("open-edit-session",{detail:{session:s},bubbles:!0,composed:!0}))}render(){const s=X(new Date),e=new Map(this.instruments.map(m=>[m.id,m])),t=this.instruments.filter(m=>!m.archived),r=!this.instFilter.length,n=r?t.map(m=>m.id):this.instFilter,i=m=>n.includes(m.instrumentId),a=["S","M","T","W","T","F","S"],o=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"],l=[];if(this.scale==="week")for(let m=6;m>=0;m--){const T=D(s,-m);l.push({t0:T,t1:D(T,1),label:a[T.getDay()],strong:m===0})}else if(this.scale==="month")for(let m=29;m>=0;m--){const T=D(s,-m);l.push({t0:T,t1:D(T,1),label:m===0||m%7===1?String(T.getDate()):"",strong:m===0})}else if(this.scale==="year")for(let m=11;m>=0;m--){const T=new Date(s.getFullYear(),s.getMonth()-m,1);l.push({t0:T,t1:new Date(T.getFullYear(),T.getMonth()+1,1),label:o[T.getMonth()].charAt(0),strong:m===0})}else{const m=this.sessions.length?this.sessions.reduce((V,mt)=>Math.min(V,new Date(mt.start).getTime()),1/0):Date.now(),T=new Date(m),F=D(s,-s.getDay()),Z=[];for(let V=D(X(T),-T.getDay());V.getTime()<=F.getTime();V=D(V,7))Z.push(V);const je=Z.slice(-26);je.forEach((V,mt)=>{l.push({t0:V,t1:D(V,7),label:V.getDate()<=7?o[V.getMonth()].charAt(0):"",strong:mt===je.length-1})}),l.length===0&&l.push({t0:F,t1:D(F,7),label:"Now",strong:!0})}const c=l.map(()=>({})),d={},u={};let h=0;this.sessions.forEach(m=>{if(!i(m))return;const T=new Date(m.start).getTime();for(let F=0;F<l.length;F++)if(T>=l[F].t0.getTime()&&T<l[F].t1.getTime()){c[F][m.instrumentId]=(c[F][m.instrumentId]||0)+m.duration,d[m.instrumentId]=(d[m.instrumentId]||0)+m.duration,u[Se(X(new Date(m.start)))]=!0,h+=m.duration;break}});const p=c.map(m=>Object.values(m).reduce((T,F)=>T+F,0)),f=Math.max(1,...p),g=Math.max(6,Math.round(300/Math.max(1,l.length))),y=l.map((m,T)=>{const F=t.filter(Z=>c[T][Z.id]).map(Z=>({color:Z.color,heightPct:c[T][Z.id]/f*100}));return{t0:m.t0,t1:m.t1,label:m.label,strong:m.strong,segments:F.length?F:[{color:"#EDECE6",heightPct:2.5}]}}),w=l[0].t0.getTime(),b=l[l.length-1].t1.getTime()-w;let E=0;this.sessions.forEach(m=>{if(!i(m))return;const T=new Date(m.start).getTime();T>=w-b&&T<w&&(E+=m.duration)});const O={week:"week",month:"30 days",year:"year",all:"period"},$=E?Math.round((h-E)/E*100):null,R=$===null?h?`first ${O[this.scale]} of data`:"no sessions yet":`${$>=0?"+":""}${$}% vs previous ${O[this.scale]}`,W=$===null?"#A3A297":$>=0?"#6B7F6E":"#9A6F6F",z={};this.sessions.forEach(m=>{i(m)&&(z[Se(X(new Date(m.start)))]=!0)});let Qs=0,vs=z[Se(s)]?s:D(s,-1);for(;z[Se(vs)];)Qs++,vs=D(vs,-1);const qt=Object.keys(u).length,jn=this.scale==="week"?`${qt} of 7 days`:this.scale==="month"?`${qt} of 30 days`:this.scale==="year"?`${qt} active days`:`${qt} active days`,Zs=10,Fn=D(s,-s.getDay()),Nn=t.map(m=>{const T=[];for(let yt=Zs-1;yt>=0;yt--){const Wt=D(Fn,-7*yt).getTime(),Bn=Wt+7*864e5;let tr=0;this.sessions.forEach(ws=>{if(ws.instrumentId!==m.id)return;const sr=new Date(ws.start).getTime();sr>=Wt&&sr<Bn&&(tr+=ws.duration)}),T.push(tr)}const F=Math.max(1,...T),Z=T.map((yt,Wt)=>`${(Wt*(100/(Zs-1))).toFixed(1)},${(31-yt/F*28).toFixed(1)}`).join(" "),je=d[m.id]||0,V=h?Math.round(je/h*100):0,mt=r||this.instFilter.includes(m.id);return{id:m.id,name:m.name,color:m.color,mins:je,minsLabel:je?ot(je):"—",sharePct:V,shareLabel:`${V}%`,spark:Z,opacity:mt?1:.32}}),Ln={week:"THIS WEEK",month:"LAST 30 DAYS",year:"LAST 12 MONTHS",all:"ALL TIME"},Un={week:"Daily minutes",month:"Daily minutes",year:"Monthly minutes",all:"Weekly minutes"},Mn=`${l[0].t0.toLocaleDateString([],{month:"short",day:"numeric"})} – ${s.toLocaleDateString([],{month:"short",day:"numeric"})}`,er=this.sessions.filter(m=>i(m)).slice(0,8);return v`
      <!-- Header -->
          ...
          ${y.map((m,T)=>v`
              <div class="chart-bar-col">
                ${m.segments.map(F=>v`
                    <span
                      class="chart-bar-seg"
                      style="height: ${F.heightPct}%; background: ${F.color}; animation-delay: ${T*g}ms;"
                    ></span>
                  `)}
              </div>
            `)}
    ...
    `];zt([P({type:Array})],Je.prototype,"instruments",2);zt([P({type:Array})],Je.prototype,"sessions",2);zt([A()],Je.prototype,"scale",2);zt([A()],Je.prototype,"instFilter",2);Je=zt([pe("pt-data-view")],Je);var ql=Object.defineProperty,Wl=Object.getOwnPropertyDescriptor,gt=(s,e,t,r)=>{for(var n=r>1?void 0:r?Wl(e,t):e,i=s.length-1,a;i>=0;i--)(a=s[i])&&(n=(r?a(e,t,n):a(n))||n);return r&&n&&ql(e,t,n),n};let Ie=class extends G{constructor(){super(...arguments),this.syncStatus="local",this.lastSyncedAt=null,this.errorMessage=null,this.popoverOpen=!1,this.isManualSyncing=!1}togglePillPopover(s){s.stopPropagation(),this.popoverOpen=!this.popoverOpen}closePopover(){this.popoverOpen=!1}async handleSyncNow(){if(!this.isManualSyncing){this.isManualSyncing=!0;try{await C.syncWithCloud(!0)}finally{this.isManualSyncing=!1}}}handleOpenSettings(){this.closePopover(),this.dispatchEvent(new CustomEvent("open-settings",{bubbles:!0,composed:!0}))}render(){const s=this.syncStatus==="syncing"||this.isManualSyncing,e=this.syncStatus==="synced"?"Synced":s?"Syncing...":this.syncStatus==="offline"?"Offline":this.syncStatus==="error"?"Sync Error":"Local Only",t=hi(this.lastSyncedAt);return v`
      <div
  ...
  `;gt([P({type:String})],Ie.prototype,"syncStatus",2);gt([P({type:String})],Ie.prototype,"lastSyncedAt",2);gt([P({type:String})],Ie.prototype,"errorMessage",2);gt([A()],Ie.prototype,"popoverOpen",2);gt([A()],Ie.prototype,"isManualSyncing",2);Ie=gt([pe("pt-sync-pill")],Ie);var Vl=Object.defineProperty,Kl=Object.getOwnPropertyDescriptor,Ye=(s,e,t,r)=>{for(var n=r>1?void 0:r?Kl(e,t):e,i=s.length-1,a;i>=0;i--)(a=s[i])&&(n=(r?a(e,t,n):a(n))||n);return r&&n&&Vl(e,t,n),n};let ke=class extends G{constructor(){super(...arguments),this.instruments=[],this.open=!1,this.selectedInstrumentId="",this.durationMinutes=20,this.sessionDate=new Date().toISOString().slice(0,10),this.notes=""}connectedCallback(){super.connectedCallback(),this.instruments.length>0&&!this.selectedInstrumentId&&(this.selectedInstrumentId=this.instruments[0].id)}willUpdate(s){s.has("instruments")&&this.instruments.length>0&&!this.selectedInstrumentId&&(this.selectedInstrumentId=this.instruments[0].id)}setPreset(s){this.durationMinutes=s}handleSave(){if(!this.selectedInstrumentId||this.durationMinutes<=0)return;const s=this.sessionDate.split("-"),e=new Date;s.length===3&&e.setFullYear(parseInt(s[0]),parseInt(s[1])-1,parseInt(s[2])),this.dispatchEvent(new CustomEvent("save-manual-session",{detail:{instrumentId:this.selectedInstrumentId,start:e,duration:this.durationMinutes,notes:this.notes},bubbles:!0,composed:!0})),this.close()}close(){this.dispatchEvent(new CustomEvent("close-modal",{bubbles:!0,composed:!0}))}render(){return this.open?v`
    ...
    `];De([P({type:Array})],he.prototype,"instruments",2);De([P({type:Object})],he.prototype,"session",2);De([P({type:Boolean})],he.prototype,"open",2);De([A()],he.prototype,"instrumentId",2);De([A()],he.prototype,"duration",2);De([A()],he.prototype,"sessionDate",2);De([A()],he.prototype,"notes",2);he=De([pe("pt-edit-session-modal")],he);var Yl=Object.defineProperty,Xl=Object.getOwnPropertyDescriptor,Xe=(s,e,t,r)=>{for(var n=r>1?void 0:r?Xl(e,t):e,i=s.length-1,a;i>=0;i--)(a=s[i])&&(n=(r?a(e,t,n):a(n))||n);return r&&n&&Yl(e,t,n),n};let xe=class extends G{constructor(){super(...arguments),this.instrument=null,this.canDelete=!0,this.open=!1,this.name="",this.color=Et[0],this.tier="secondary"}willUpdate(s){s.has("instrument")&&this.instrument&&(this.name=this.instrument.name,this.color=this.instrument.color,this.tier=this.instrument.tier)}handleSave(){if(!this.instrument||!this.name.trim())return;const s={...this.instrument,name:this.name.trim(),color:this.color,tier:this.tier};this.dispatchEvent(new CustomEvent("update-instrument",{detail:{instrument:s},bubbles:!0,composed:!0})),this.close()}handleDelete(){this.instrument&&confirm(`Remove "${this.instrument.name}" from kit? Past practice history will be preserved.`)&&(this.dispatchEvent(new CustomEvent("remove-instrument",{detail:{instrumentId:this.instrument.id},bubbles:!0,composed:!0})),this.close())}close(){this.dispatchEvent(new CustomEvent("close-modal",{bubbles:!0,composed:!0}))}render(){return!this.open||!this.instrument?v``:v`
      <div class="modal-overlay" @click=${s=>s.target===s.currentTarget&&this.close()}>
        <div class="modal-card">
          <div class="modal-title">
